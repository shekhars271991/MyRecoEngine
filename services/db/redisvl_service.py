...
try:
    ...
    user_movie_index = SearchIndex.from_dict(user_movie_schema)
    user_movie_index.set_client(redis_client)

# product indices
    product_index = SearchIndex.from_dict(product_schema)
    product_index.set_client(redis_client)
    user_product_index = SearchIndex.from_dict(user_product_schema)
    user_product_index.set_client(redis_client)
...
def create_user_movie_index(overwrite=False):
    """
    Creates the user index in Redis.
    :param overwrite: If True, overwrites the existing index.
    """
    try:
        user_movie_index.create(overwrite=overwrite)
...
def search_similar_movie_users(query_vector,num_results=10):
    try:
        # Convert the query vector to bytes
        vector = np.array(query_vector, dtype=np.float32).tobytes()
        ...
        # Perform the query
        results = user_movie_index.query(vq)
        return results
...
def initialize():
    ...
    try:
        ...
        create_user_movie_index(overwrite=False)
        create_product_index(overwrite=False)
        create_user_product_index(overwrite=False)
    except Exception as e:
...
def create_product_index(overwrite=False):
    """
    Creates the product index in Redis.
    :param overwrite: If True, overwrites the existing index.
    """
    try:
        product_index.create(overwrite=overwrite)
        print("Product index created successfully.")
    except Exception as e:
        print(f"Failed to create product index: {e}")
        raise

def create_user_product_index(overwrite=False):
    """
    Creates the user product index in Redis.
    :param overwrite: If True, overwrites the existing index.
    """
    try:
        user_product_index.create(overwrite=overwrite)
        print("User product index created successfully.")
    except Exception as e:
        print(f"Failed to create user product index: {e}")
        raise




def search_products_by_vector(query_vector, categories=None, min_price=None, max_price=None, num_results=10):
    """
    Performs a vector search with optional filters on genres and release year.
    ...
    try:
        ...
        # Perform the query
        results = product_index.query(vq)
        return results
    except (RedisError, ConnectionError) as e:
        print(f"Redis query failed: {e}")
        raise
...
def search_similar_product_users(query_vector, index_name='user_profiles', num_results=10):
    try:
    # Convert the query vector to bytes
        vector = np.array(query_vector, dtype=np.float32).tobytes()
        # Create a VectorQuery
        ...
        # Perform the query
        results = user_product_index.query(vq)
        return results
